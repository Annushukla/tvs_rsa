...
Class Rsa_cover extends CI_Controller {
    ...
    function index($policyid = NULL) {
        ...
        if(!empty($policyid)){
            $this->data['policy_data'] = $policy_data = $this->HomeMdl->getPolicyById($policyid);
            $veh_reg_no = explode('-', $policy_data['vehicle_registration_no']);
            ...
            $this->data['vehicle_type'] = $policy_data['vehicle_type'];
            $this->data['engine_no'] = $policy_data['engine_no'];
            $this->data['chassis_no'] = $policy_data['chassis_no'];
            $this->data['pincode'] = $policy_data['pincode'];
            $this->data['city'] = $policy_data['city'];
            $this->data['state'] = $policy_data['state'];
            $this->data['addr1'] = $policy_data['addr1'];
            $this->data['addr2'] = $policy_data['addr2'];
            $this->data['dob'] = $policy_data['dob'];
            $this->data['gender'] = $policy_data['gender'];
            $this->data['mobile_no'] = $policy_data['mobile_no'];
            $this->data['email'] = $policy_data['email'];
            $this->data['fname'] = $policy_data['fname'];
            $this->data['lname'] = $policy_data['lname'];
            $this->data['nominee_full_name'] = $policy_data['nominee_full_name'];
            $this->data['nominee_relation'] = $policy_data['nominee_relation'];
            $this->data['nominee_age'] = $policy_data['nominee_age'];
            $this->data['appointee_full_name'] = $policy_data['appointee_full_name'];
            $this->data['appointee_relation'] = $policy_data['appointee_relation'];
            $this->data['appointee_age'] = $policy_data['appointee_age'];
            $this->data['plan_type_id'] = $policy_data['plan_type_id'];
            $this->data['plan_id'] = $policy_data['plan_id'];
            $this->data['policy_id'] = $policy_data['policy_id'];
...
$i = 1;
            ...
            foreach ($plan_details as $plan_detail) {
                ...
                $lable = '₹ ' .$plan_code;
                $disabled = ($age>64 && $plan_code == 678)?'disabled':'';
                $html .=<<<EOD
                        <tr>
                          <th scope="row">{$i}</th>
                          <td>{$plan_name}</td>
                          ...
                          <td>Current</td>
                          <td>{$lable}</td>
                          <td><input type="radio" name="plan" id="{$plan_code}" value="{$id}" data-plan="{$plan_amount}" {$checked}{$disabled}></td>
                        </tr>
EOD;
            $i++;
            }
...
//        $sql = "$sql WHERE  1 = 1";
        ...
        if (!empty($engine_no)) {
            ...
            if ($status) {
                ...
                if (!empty($policy_data)) {
                    ...
                    $owner_gender = (!empty($policy_data['Owner']['Owner_Gender']))?$policy_data['Owner']['Owner_Gender']:'';
                    $gender = ($owner_gender == 'F')?'female':'male';
                    $dob_ar = explode(' ', $policy_data['Owner']['owner_DOB']);
                    // echo '<pre>'; print_r($dob_ar);
                    $dob = (!empty($dob_ar[0]))?date("m/d/Y", strtotime($dob_ar[0])):'';
                    $name = explode(' ', $name);
                    $name = array_values(array_filter($name));
                    $fname = !empty($name[0])?$name[0]:'';
                    $mname = isset($name[2])?(isset($name[1])?$name[1]:''):'';
                    ...
                    $customer_and_vehicle_details = array(
                        ...
                        'registration_date' => ($policy_data['vehicle']['registration_date'])?($policy_data['vehicle']['registration_date']):'',
   ...
   function CheckExistFrameNo() {
        $engine_no = $this->input->post('vehicle_detail');
        if (!empty($engine_no)) {
            $vehicle_detail = $engine_no;
                $crc_data = $this->getEngineNoFromDms(strtoupper($engine_no));
                $engine_no = !empty($crc_data['data']['ENGINE_NO'])?$crc_data['data']['ENGINE_NO']:'';
                if(!empty($engine_no)){
                $policy_data = $this->getTvsRsaPolicyData(strtoupper($engine_no));
                if (!empty($policy_data)) {
                    $result['status'] = 'new_policy';
  ...
  function getTvsRsaPolicyData($engineNo) {
    ...
    function getEngineNoFromDms($engineNo) {
        $newArray = array();
        //tokenexe='INSR201808160900|87000001|16/08/2018'
        $currentDate = date("d/m/Y");
        $data = '';
        ...
        $soap_body = '<Envelope xmlns="http://schemas.xmlsoap.org/soap/envelope/">
            <Body>
                <get_CustomerCRC_details xmlns="http://tempuri.org/">
                    <id><![CDATA['.$dataString.']]></id>
                </get_CustomerCRC_details>
            </Body>
        </Envelope>';
        // echo $soap_body;
        $curl = curl_init();
    ...
    $response = curl_exec($curl);
     // echo "<pre>"; print_r($response); echo "</pre>";// die('end of line yoyo');
        
        $file_contents = str_replace('<?xml version="1.0" encoding="utf-8"?><soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema"><soap:Body><get_CustomerCRC_detailsResponse xmlns="http://tempuri.org/"><get_CustomerCRC_detailsResult><soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema"><soap:Body><get_CustomerCRCResponse xmlns="http://tempuri.org/">', "", $response);

        $file_contents = str_replace('</get_CustomerCRCResponse></soap:Body></soap:Envelope></get_CustomerCRC_detailsResult></get_CustomerCRC_detailsResponse></soap:Body></soap:Envelope>', "", $file_contents);
   ...
   function checkIsAvailableForWorkshopPolicy(){
            $engine_no = $this->input->post('engine_no');
            $status = $this->HomeMdl->checkIsAvailableForWorkshopPolicy($engine_no);
            $return_data['status'] = ($status)?'true':'false';
...
function GeneratedPolicyData() {
        ...
        $age =  date_diff(date_create($dob), date_create('today'))->y;
         // echo "<pre>"; print_r($post_data); echo "</pre>".$age;die;
        // die($age);
        $page_name = !empty($post_data['page_name'])?$post_data['page_name']:'';
        $policy_id = $post_data['policyid'];
        if($age > 74 || $age < 18){
            $this->session->set_flashdata('message', 'Customer Age Should Not More Than 75 Or Less Than 16.');
            if($page_name == 'renewal'){
                redirect('renew_policy/'.$policy_id);
            }else{
        ...
        if (empty($policy_id) || $page_name == 'renewal') {
                ...
                if($wallet_balance < ($plan_details['plan_amount'] + $plan_details['gst_amount'])){
                   ...
                   // print $is_exist;exit;
                    if($is_exist){
                       ...
                       $last_name = !empty($this->input->post('middel_name'))?($this->input->post('middel_name').' '.$this->input->post('last_name')):$this->input->post('last_name');
                       $insert_customer_detail = array(
                            'fname' => trim($this->input->post('first_name')),
                            'mname' => trim($this->input->post('middel_name')),
                            ...
                            'pincode' => trim($this->input->post('pin')),
                            'nominee_full_name' => trim($this->input->post('nominee_full_name')),
                            'nominee_relation' => $nominee_relation,
                            'nominee_age' => trim($this->input->post('nominee_age')),
                            'appointee_full_name' => trim($this->input->post('appointee_full_name')),
                            'appointee_relation' => trim($this->input->post('appointee_relation')),
                            'appointee_age' => trim($this->input->post('appointee_age')),
                        ...
                        // echo 'customer details Inseerted';
                        ...
                        if ($customer_detail_last_id) {
                            ...
                            $pa_effective_date = $effective_date;
                            $pa_end_date = date('Y-m-d', strtotime(date('Y-m-d', strtotime($pa_effective_date . "-1 day")) . "1 year"));
                            ...
                            if(in_array($post_data['plan'], array(51,52))){
                                ...
                                if(!empty($getic_map['pa_ic_id'])){
                                    ...
                                    $dealer_ic_details = $this->HomeMdl->getRowDataFromTable('tvs_dealers_dmsic_logic',array('dealer_code'=>$dealer_code));
                                    $pa_ic_id = $this->getIcIdByShortName($dealer_ic_details['pa_ic_mapping']);
                                 }
                            $pa_ic_id = ($age >64 &&  (!in_array($pa_ic_id, array(12,13))))?13:$pa_ic_id;    
                            switch ($pa_ic_id) {
                                    case 10:
                                        ...
                                        break;
                                    default:
                                       $master_policy_details = $this->HomeMdl->getRowDataFromTable('ic_pa_master_policy_nos',array('ic_id' => $pa_ic_id));
                                        break;
                                }
                         }
                        ...
                        if (!empty($inserted_id)) {
                            ...
                            $this->allTransectionInsert($user_data,$result_sold,$transection_no,$plan_details);
                            $domainName = $_SERVER['HTTP_HOST'];
                            $this->data['url']='generate_policy';
                            if(in_array($post_data['plan'], array(51,52))){
                                ...
                                $this->load->dashboardTemplate('front/myaccount/success', $this->data);
                                    ...
                                    if ($domainName != 'localhost' && !empty($post_data['email'])) {
                                        $this->MailSoldPolicyPdf($inserted_id,$pa_ic_id);
                                    }
                                    $ic_pdf = $this->getPdfNameByPolicyId($pa_ic_id);
                                    $this->data['ic_pdf'] = $ic_pdf;
                                    $this->data['inserted_id'] = $inserted_id;
                                    $this->load->dashboardTemplate('front/myaccount/success', $this->data);
            ...
            $policy_details = $this->HomeMdl->getRowDataFromTable('tvs_sold_policies',$where);
               ...
               $update_customer_detail = array(
                    ...
                    'nominee_full_name' => trim($post_data['nominee_full_name']),
                    'nominee_relation' => $nominee_relation,
                    'nominee_age' => trim($post_data['nominee_age']),
                    'appointee_full_name' => isset($post_data['appointee_full_name']) ? trim($post_data['appointee_full_name']) : '',
                    'appointee_relation' => isset($post_data['appointee_relation']) ? $post_data['appointee_relation'] : '',
                    'appointee_age' => isset($post_data['appointee_age']) ? trim($post_data['appointee_age']) : '',
  ...
  function getPdfNameByPolicyId($ic_id){
                switch ($ic_id) {
                        case 5:
                        $ic_pdf = 'download_il_lite_pdf';
                            break;
                        case 7:
                        $ic_pdf = 'download_hdfc_policy';
                            break;
                        case 8:
                        $ic_pdf = 'download_reliance_policy';
                            break;
                        case 9:
                            $ic_pdf = 'download_tata_lite_pdf';
                            break;
                        case 12:
                            $ic_pdf = 'download_bagi_lite_pdf';
                            break;
                        case 13:
                            $ic_pdf = 'download_liberty_policy';
                            break;
                        default:
                            $ic_pdf = 'download_OICL_pdf';
                            break;
                    }
            return $ic_pdf;
   }

  function allTransectionInsert($user_data,$result_sold,$transection_no,$plan_details){
          $dealer_transection_data = array(
            ...
            if($status){
                $policy_amount = ($plan_details['plan_amount']+$plan_details['gst_amount']);
                $wallet_details = $this->HomeMdl->getRowDataFromTable('dealer_wallet',array('dealer_id'=>$user_data['id']));
                $dealer_details = $this->HomeMdl->getRowDataFromTable('tvs_dealers',array('id'=>$user_data['id']));
                if($dealer_details['is_allowed_commission_to_bank'] == 1){
                     $wallet_amount = (($wallet_details['credit_amount'] + $policy_amount));
                     $total_commission = (($wallet_details['total_commission'] + $plan_details['dealer_commission']));
                    $data = array(
   ...
   function getIcIdByShortName($short_name){
            switch ($short_name) {
                    case 'OICL':
   ...
   function StartEndDate($post_data,$plan_details){
       $where =  array('engine_no'=>$post_data['engine_no'],'policy_status_id'=>3);
       $is_policy_exist = $this->db->select('*')->from('tvs_sold_policies')->where($where)->get()->row();
      // echo '<pre>';       print_r($is_policy_exist['sold_policy_end_date']);die('here');
       $pre_policy_expiry_date = !empty($is_policy_exist->sold_policy_end_date)?$is_policy_exist->sold_policy_end_date:date('Y-m-d');
     if($post_data['plan_type'] == 1){
        ...
        $start_date = ($pre_policy_expiry_date <= $start_date) ? $start_date : date('Y-m-d', strtotime($pre_policy_expiry_date . "1 day"));
...
//$pa_sold_policy_effective_date = $date->format('d-M-Y').' 12:00:01 AM';
...
//$pa_sold_policy_end_date = $date->format('d-M-Y').' 11:59:59 PM';
      ...
      <td>
        <table cellpadding="2" border="0" cellspacing="0" class="boxtable">
          <tr>
            ...
            <td width="32%"><b>Premium frequency:</b> </td>            
          </tr> 
        </table>
        <table cellpadding="2" border="0" cellspacing="0" class="boxtable">
            <tr>
        ...
        <table cellpadding="2" border="0" cellspacing="0" class="boxtable ">
          <tr>
            <td>1. The personal accident coverage is applicable only to the owner of the vehicle for which Road Side Assistance service has been provided by the Indicosmic Capital Pvt Ltd. <br>2. The said personal accident cover is active only whilst the owner is driving or travelling in the said vehicle including mounting into/dismounting from the said vehicle. <br>3. Death or permanent total disability claims due to any other incidence would not be covered. <br>4. The policy is valid for 365 days from the policy risk start date.</td>
          </tr>
        </table>   
        <table cellpadding="0" border="0" cellspacing="0">
            <tr>
...
$result=$pdf->Output('HDFC-ERGO-GPA-Policy.pdf', 'S');
        ...
        <tr>
          ...
          <td width="35%">
            <table cellpadding="1" border="0" cellspacing="0">
              ...
              <tr>
                <td width="20%"><b>Address:</b></td>
                <td width="80%">Bharti Assist Global Private Limited Bharti Crescent, 1 Nelson Mandela RoadVasant Kunj, New Delhi 110070</td>
              </tr>
              <tr>
                <td><b>Email:</b></td>
                <td>tvsmotors@bhartiassistance.co.in</td>
              </tr>
            </table>
          </td>
        </tr> 
  ...
  <tr>
    <td>
      ...
      <table cellpadding="1" border="0" cellspacing="0">
        ...
        <tr>
          <td>Beneficiary's Address: {$addr1} </td>
        </tr>
        <tr>
...
//        $sql = "$sql WHERE  1 = 1";
    ...
    function cancleRsaPolicyAjax() {
        ...
        if(!empty($from_date) && !empty($to_date)){
            $date_where=" AND DATE(tsp.created_date) BETWEEN DATE('$from_date') AND DATE('$to_date') ";
        }else{
            $date_where = " AND DATE(tsp.created_date) >(NOW() - INTERVAL 15 DAY) " ;
        }
        $where = '';
        if(strlen($employee_code) > 5){
            $where = 'AND employee_code = '.$employee_code.'';
        ...
        $sql = "SELECT tsp.id,tsp.policy_status_id,tsp.product_type_name,tsp.plan_name,tsp.sold_policy_no,tsp.engine_no,
                tsp.chassis_no,tsp.created_date,tcd.fname FROM tvs_sold_policies AS tsp
                LEFT JOIN tvs_customer_details AS tcd ON tcd.id = tsp.customer_id WHERE tsp.user_id = '$user_id' $where $date_where ";
...
// echo $created_date[0];die('abc');
            ...
            $pa_ic = "";
            //print 'Rsa ic id '.$main->rsa_ic_id;exit;
            if($main->plan_id==62 || $main->plan_id==63 || $main->plan_id==64){
                $file_list = '<a href="' . base_url() . 'download_rsa_pdf/' . $main->policy_id. '" class="btn btn-info" target="_blank">Download Pdf</a>';
                    $pa_ic = "";
            }
            ...
            switch ($main->ic_id) {
                ...
                case 5:
                    ...
                    break;
                case 7:
                    $file_list = '<a href="' . base_url() . 'download_hdfc_policy/' . $main->policy_id . '" class="btn btn-info" target="_blank">Download Pdf</a>';
                    $pa_ic = "HDFC";
                    break;
                case 8:
            ...
            if($main->plan_id==65 && $main->ic_id==10 ){
                $file_list = '<a href="' . base_url() . 'download_workshop_OICL_pdf/' . $main->policy_id. '" class="btn btn-info" target="_blank">Download Pdf</a>';
                    $pa_ic = "Oriental";
            }
            $row = array();
            $row[] = $i++;
...
//        $sql = "$sql WHERE  1 = 1";
    ...
    public function rsaPaRenewalSendSms($interval){
        $policy_expiry_customers = $this->HomeMdl->getPolicyExpiryCustomersData($interval);
      //  echo '<pre>';        print_r($policy_expiry_customers);die('hello moto');
        if(!empty($policy_expiry_customers)){
            foreach ($policy_expiry_customers as $key => $policy_expiry_customer) {
                 echo '<pre>';        print_r($policy_expiry_customer);
                $data['cust_name'] = $cust_name = !empty($policy_expiry_customer->customer_name)?$policy_expiry_customer->customer_name:'Customer';
                $data['policy_no'] = $policy_no = !empty($policy_expiry_customer->sold_policy_no)?$policy_expiry_customer->sold_policy_no:'NAN';
                $data['start_date'] = $start_date = !empty($policy_expiry_customer->start_date)?$policy_expiry_customer->start_date:'0000-00-00';
                $data['end_date'] = $end_date = !empty($policy_expiry_customer->end_date)?$policy_expiry_customer->end_date:'0000-00-00';
                $data['policy_id'] = $policy_id = !empty($policy_expiry_customer->policy_id)?$policy_expiry_customer->policy_id:'';
                $data['mobile_no'] = $mobile_no = '9664291524';//!empty($policy_expiry_customer->mobile_no)?$policy_expiry_customer->mobile_no:'';
                 echo '<pre>';        print_r($data);
                 //die('hello moto');
//                if(!empty($mobile_no)){
//                   $is_sms_sent = $this->sendSmsForRsaPaRenewal($data);
...
//                   }
//                }else{
//                    echo 'Mobile No Not Exist';
//                }
            }
        }
        
    }
function sendSmsForRsaPaRenewal($data){
        $base_url = base_url('renewRsaPaPolicy').'/'.$data['policy_id'];
        $customer_name = $data['cust_name'];
        $policy_no = $data['policy_no'];
        $start_date = $data['start_date'];
        $end_date = $data['end_date'];
        $mobile_no = $data['mobile_no'];
        $message = "Dear $customer_name,Your policy is expiring on $end_date to renew kindly click on given url ".$base_url;
        $message = urlencode($message);
        $output = $this->CurlSMS($message,$mobile_no);
        return $output;
    }
    
    function CurlSMS($message,$mobile_no) { 
    $uid = "MPOLNW";
    $pwd = 'Pwe$NiTm';
    $sid = "TVSRSA";
    $method = "GET";
    $get_url = "http://123.108.46.12/API/WebSMS/Http/v1.0a/index.php?username=".$uid."&password=".$pwd."&sender=TVSRSA&to=".$mobile_no."&message=".$message."&reqid=1&format=text";
    ...
    function renewRsaPaPolicy($policy_id){
        if(!empty($policy_id)){
            $data['policy_data'] = $this->HomeMdl->getPolicyById($policy_id);
            $dat['plan_details']=$plan_details = $this->getPlanDetails($data['policy_data']);
        ...
        // $data['main_contain'] = 'admin/report/buy_pending_policy';
        // $this->load->view('admin/includes/api_template', $data);

        $this->load->dashboardTemplate('front/myaccount/api_rsa_cover',$data);
    }

function getPlanDetails($policy_data){
    $plan_type_id = 1;
    // echo '<pre>'; print_r($policy_data);die;
...
function renewalPolicy(){
    $this->load->dashboardTemplate('front/myaccount/renewal_policy',$this->data);
}

function renewalPolicyAjax(){
    // echo '<pre>'; print_r($_REQUEST);die('here');
        $interval = $_REQUEST['interval'];
        $length = $_REQUEST['length'];
        $columns = array();
        $dealer_id = $this->session->userdata('user_session')['id'];
        $pending_policies = $this->HomeMdl->getPolicyExpiryCustomersData($dealer_id,$interval);
      //   echo '<pre>'; print_r($pending_policies);die('here');
        $totalFiltered = !empty($pending_policies)?count($pending_policies):0;
        $data = array();
        
        $k=1;
        if(!empty($pending_policies)){
        foreach ($pending_policies as $main) {
          $action='<a href="' . base_url() . 'renew_policy/' . $main->policy_id . '" class="btn btn-info">Renew Policy</a>';
            $row = array();
            $row[] = $k;
            $row[] = trim($main->sold_policy_no);
            $row[] = $main->customer_name;
            $row[] = $main->ic_name;
            $row[] = $main->engine_no;
            $row[] = $main->chassis_no;
            $row[] = date('Y-m-d',strtotime($main->start_date));
            $row[] = date('Y-m-d',strtotime($main->end_date));
            $row[] = date('Y-m-d',strtotime($main->created_date));
            $row[] = $action;
...
function renewPolicy($policyid){
     $user_session = $this->session->userdata('user_session');
        ...
        if(!empty($policyid)){
            ...
            $this->data['policy_data'] = $policy_data = $this->HomeMdl->getPolicyById($policyid);
            $status = $this->HomeMdl->checkDuplicatePolicy($policy_data['engine_no']);
            if($status){
                $this->session->set_flashdata('message', 'Policy Is Already Created For That Vehicle.');
                redirect('renewal_policy');
            }
            $veh_reg_no = explode('-', $policy_data['vehicle_registration_no']);
            $this->data['rto_name'] = $veh_reg_no[0];
            $this->data['rto_code1'] = $veh_reg_no[1];
            ...
            $this->data['email'] = $policy_data['email'];
            $this->data['fname'] = $policy_data['fname'];
            $this->data['mname'] = $policy_data['mname'];
            $this->data['lname'] = $policy_data['lname'];
            $this->data['nominee_full_name'] = $policy_data['nominee_full_name'];
            $this->data['nominee_relation'] = $policy_data['nominee_relation'];
            $this->data['nominee_age'] = $policy_data['nominee_age'];
        ...
        if(!empty($user_session['dealer_code'])){
            $this->load->dashboardTemplate('front/myaccount/renew_rsa_cover', $this->data);
        }else{
            redirect('');
        }
...
function ExpiredPolicies(){
    $user_session = $this->session->userdata('user_session');
    $sap_ad_code = $user_session['sap_ad_code'];
    if ( (strlen($sap_ad_code) ==5) ) {
            $this->load->dashboardTemplate('front/myaccount/expired_policies');
        } else {
            redirect('dashboard');
        }
}

function DealerExpiredPolicyAjax(){
    $length = $_REQUEST['length'];
    $columns = array();
    ...
    $engine_no = $this->input->post('engine_no');
    $dealer_id = $this->session->userdata('user_session')['id'];
    $pending_policies = $this->HomeMdl->getDealerExpiredPolicies($dealer_id,$start_date,$end_date,$engine_no);
    // echo '<pre>'; print_r($pending_policies);die('here');
    $totalFiltered = $pending_policies['num_rows'];
    $data = array();
    
    $k=1;
    if(!empty($pending_policies)){
    foreach ($pending_policies['result'] as $main) {
      $action='<a href="' . base_url() . 'renew_policy/' . $main['sold_id'] . '" class="btn btn-info">Renew Policy</a>';
        $row = array();
        $row[] = $k;
        $row[] = trim($main['sold_policy_no']);
        $row[] = $main['customer_name'];
        ...
        $row[] = $main['engine_no'];
        $row[] = $main['chassis_no'];
        $row[] = date('Y-m-d',strtotime($main['sold_policy_effective_date']));
        $row[] = date('Y-m-d',strtotime($main['sold_policy_end_date']));
        $row[] = date('Y-m-d',strtotime($main['created_date']));
        $row[] = $action;
