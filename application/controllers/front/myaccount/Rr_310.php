...
Class Rr_310 extends CI_Controller {
    ...
    function index() {
        ...
        $this->load->view('front/myaccount/rr_310', $this->data);
    }

    function disclaimer(){
        $this->load->view('front/myaccount/disclaimer', $this->data);
    }        

    function rr_310_policy(){
        $rsa_type = $this->input->post('rsa_type');
        if($rsa_type == 'rr_310'){
            $dealer_details = $this->HomeMdl->getDealersDetails();
            $this->data['dealer_details'] = $dealer_details;
        }
        // echo '<pre>'; print_r($dealer_details);die('hello moto');
        $user_session = $this->session->userdata('user_session');
       if(!empty($dealer_details)){
            $this->load->dashboardTemplate('front/myaccount/rsa_rr310_policy', $this->data);
        }else{
            redirect('Rr_310');
        } 
    }

    function rr310GeneratePolicy() {
        $dealer_details = $this->HomeMdl->getDealersDetails();
        $post_data = $this->input->post();
        $engine_no = $post_data['engine_no'] ;
        $chassis_no = $post_data['chassis_no'];
        $exist_policydata = $this->HomeMdl->RR310checkIsPolicyExist($engine_no,$chassis_no);
        // echo "<pre>"; print_r($exist_policydata); echo "</pre>"; die('end of line yoyo');
        if($exist_policydata){
            $this->session->set_flashdata('message','Engine No. Or Chassis No. Have Been Already Used ,Please Contact To Customer Service !');
            redirect('Rr_310');
        }
        ...
        if (empty($policy_id)) {
            $this->form_validation->run('rr310_rsa_form');
                    ...
                    if($is_exist){
                       ...
                       $insert_customer_detail = array(
                        ...
                        if ($customer_detail_last_id) {
                            ...
                            // print_r($result_sold);die('result_sold');
                            ...
                            // Get Rsa_ic_id from state id 
                            $where = array('state_id'=> $this->input->post('state_id'));
                            $rsa_id_detail=$this->HomeMdl->getRowDataFromTable('state_rsa_mapping',$where);
                            ...
                            $insert_data_sold = array(
                                ...
                                //'rsa_ic_id' => 11,
                                'rsa_ic_id' => $rsa_id_detail['rsa_ic_id'],
                                'vehicle_type' => '',
                                'sold_policy_date' => date('Y-m-d H:i:s'),
                                'created_date' => date('Y-m-d H:i:s')
    ...
    function checkPayment($transection_no,$inserted_id,$post_data,$user_data){
            ...
            $paramList = array(
                ...
                "CALLBACK_URL" => base_url(CALLBACK_URL).'/'.$transection_no.'/'.$inserted_id
            );
    ...
    function paymentResponse($transection_no,$inserted_id){
        $paytmChecksum = "";
        $paramList = array();
        ...
        if($isValidChecksum){
            ...
            $res_payment_response=$this->HomeMdl->insertIntoTable('tvs_payment_response',$insert_payment_response);
            if ($_POST["STATUS"] == "TXN_SUCCESS") {
                ...
                $res_upd_sold_policies=$this->updateTable('tvs_sold_policies',$input_data,$where);
                ...
                if($res_upd_sold_policies){
                    ...
                    if($paymentStatus['STATUS'] == 'TXN_SUCCESS'){
                            ...
                            $this->updateTable('tvs_payment_response',$input_data,$where);
                            $this->session->set_userdata('policy_id',$inserted_id);
                        $sql = "SELECT tcs.`mobile_no` ,tcs.`email`,tcs.`fname` AS name,tsp.`sold_policy_no` as policy_no FROM tvs_sold_policies tsp JOIN tvs_customer_details tcs ON tsp.`customer_id`= tcs.`id` WHERE tsp.`id`='$inserted_id'";
                        ...
                        $is_sent = $this->RR310_sendSms($name,$policy_no,$Mobilenumber,$inserted_id);
                        //$is_sent=1;
    ...
    $this->load->dashboardTemplate('front/myaccount/rsa_cover');
    ...
    function RR310_sendSms($name,$policy_no,$Mobilenumber,$sold_policy_id) { 
        $base_url = base_url().'download_rr_310_pdf/'.$sold_policy_id;
        $video_url = base_url().'paymentSuccess?id='.$sold_policy_id;
        $short_video_url = $this->make_bitly_url($video_url,'json');
        ...
        $Message = "TVS Apache RR310: Hi $name,
                    Thank you for purchasing Limitless Assist RSA policy. Your RSA policy number $policy_no is successfully generated. To download your policy, see $short_policy_url
    ...
    public function planDetailsForRR310(){
            ...
            $plan_id = ($vehicle_age > 90)?62:61;
            $where = array('id' =>$plan_id);
            $key = 'plan_amount';
            $plan_details_arr = $this->HomeMdl->getDataFromTable('tvs_plans',$where); //$query->result_array();
...
$i = 1;
            ...
            foreach ($plan_details_arr as $plan_details_arr) {
                // if($plan_details_arr['id']==61 || $plan_details_arr['id']==55){
                //     continue;
                // }
                $checked = ($plan_details_arr['id'] == $selected_plan_id)?'checked':'';
                $plan_code = ($plan_details_arr['id']==61)?0:$plan_details_arr['plan_code'];
                $id = $plan_details_arr['id'];
                $plan_amount = ($plan_details_arr['id']==61)?0:$plan_details_arr['plan_amount'];
                ...
                $html .=<<<EOD
                        <tr>
                          ...
                          <td>{$lable}</td>
                          <td><input type="radio" name="plan_rr310" id="{$plan_code}" value="{$id}" data-plan="{$plan_amount}" {$checked}></td>
                        </tr>
EOD;
            $i++;
            }
...
EOD;
    ...
    function CheckExistEngineNo_RR310_bk() {
    ...
    function CheckExistEngineNo_RR310() {
        ...
        if (!empty($engine_no)) {
            ...
            if ($status) {
                ...
                // previous using soap commented
                //$policy_data = $this->getTvsRsaPolicyData(strtoupper($engine_no));

                //new using db 25/10/2019
                $policy_data = $this->getTvsRsaPolicyDataDb(strtoupper($engine_no));
                ...
                if (!empty($policy_data)) {
                    ...
                    $customer_and_vehicle_details = array(
                        'product_type' => 1,
                        'engine_no' => trim($policy_data['engine_no']),
                        'chassis_no' => trim($policy_data['chassis_no']),
                        'manufacturer' => 11,
                        'model' => 'APACHE RR 310',
                        'color' => trim($policy_data['color']),
                        'first_name' => explode(' ',$policy_data['cust_name'])[0],
                        'last_name' => explode(' ',$policy_data['cust_name'])[2],
                        'dob'=>($dob=='01/01/1900')? '' : $policy_data['dob'],
                        'email' => trim($policy_data['email_id']),
                        'mobile_no' => trim($policy_data['mobile_no']),
                        'cust_addr1' => trim($policy_data['address1']),
                        'cust_addr2' => trim($policy_data['address2']),
                        'state' => trim($policy_data['state_id']),
                        'city' => trim($policy_data['city']),
                        'pin' => trim($policy_data['pincode']),
    ...
    public function getTvsRsaPolicyDatadb($engineNo){
       $where=array('engine_no'=>$engineNo);
       $rr310_veh_cust_data=$this->HomeMdl->getRowDataFromTable('rr310_vehicle_customer_data',$where);
       //print_R($rr310_veh_cust_data);exit;
       return $rr310_veh_cust_data;
    }
...
function DownloadOnlyRsaPolicy($id){
  ...
  $getDealerInfo = $this->HomeMdl->getOnlyRsaPolicyDealerinfo($id);
            ...
            <td style="width: 48%;">
              <table class="table" width="100%" bgcolor="" cellpadding="4" border="0" cellspacing="0" style="border-left: 1px solid #16365c; border-right: 1px solid #16365c;">
                  <tbody>
                    ...
                    <tr>
                      <td style=" height: 80px; color: #000; font: 8pt arial; line-height: 1.2; padding:5px; border-bottom: 1px solid #16365c;"><img src="" alt="" style="height:30px"><br><b>Address:</b>  {$rsa_address}</td>
                    </tr>
                </tbody>
              </table>
            </td>
...
//Close and output PDF document
   ...
   return $result;
      ...
      function MailAttachments($policypdf_obj, $sold_policy_detail_last_id) {
        ...
        $rsa_ic_id=($rsa_ic_master_id==11)?2:1;
        $where = array(
            'id' => $rsa_ic_id
        );
        ...
        //print 'Email '.$customer_mail;exit;
       ...
       /* $msg = "<html><body>
        ...
        <p>Dear $customer_name,</p>
        <p>
        Welcome to the Apache RR310 fraternity.
        Now race away with your Apache RR310 anywhere in India, hassle-free, with your Limitless Assist, unlimited KMs Road Side Assistance policy.
        Thank you for purchasing your Limitless Assist RSA policy from Authorized TVS Dealer.
          Your RSA Policy number $sold_policy_no has been successfully generated and it is attached for your reference.
          In case of any queries or assistance, please call us on $toll_free_no or write to us at $rsa_email.
          This is your original policy copy.
          Limitless Assist 24x7 emergency helpline contact number 1800 258 7111.
          Purchase and Policy terms available at https://www.tvsapache.com/rr310/.
          For more information about Limitless Assist RSA, see https://www.tvsservice.com/Rr_310 </p><br>
        ...
        //print 'Mail Status '.$result['status'];exit;
        return $result;
    }
