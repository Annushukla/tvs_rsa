...
class Admin_Model extends CI_Model {
  ...
  public function getPolicyDetails($post_data){
        ...
        OR engine_no = '$policy_no'
        OR chassis_no = '$policy_no'
        ";
        //die($sql);
   ...
   public function getNearestVendores($task_details){
    ...
    public function getNearestWorkshop($task_details){
      $lat = $task_details['lat'];      
      $longi = $task_details['longi'];
      $sql = "SELECT *, SQRT(
            POW(69.1 * (lat - $lat), 2) +
  ...
  function getDealerSoldPolicies($invoice_month){
    ...
    function getRowDataFromTableOrderByDesc($table_name = null,$key,$where = null) {
        if ($where != null) {
            $this->db->where($where);
        }
        return $result = $this->db
...
function getRSACanceledFeedfile($from_date,$to_date,$ic_id){
 ...
 public function getExistInvoiceDetails($invoice_no,$dealer_id){
     ...
     function getAllPolicyDetailsWithIc($rsa_ic_id){
     $sql = "SELECT * 
        FROM `customer_ques_ans_details` cqd
        INNER JOIN  `vendor_current_record` vcr
        ON cqd.id = vcr.`task_id`
        WHERE vcr.`active` = 1
        AND cqd.rsa_ic_id = $rsa_ic_id
        AND vcr.`task_completed_status`='pending'";
       $result =  $this->db->query($sql)->result_array();
        return $result ;

    }

    function getAllPolicyDetails($id =null){

        if(empty($id)){
            $where = '';
        }else{
            $where = "WHERE cqad.rsa_ic_id = $id";
        }
        $sql = "SELECT 
                cqad.`rsa_status_id`,
                tsd.`status_code`,
                ...
                cqad.`workshop_id`,
                cqad.`vendor_id`,
                cqad.`customer_task_complete_status`,
                cqad.`is_invoice_generated`,
                cqad.`created_at`,
                cqad.`vendor_task_completed_status`,
                cqad.`vendor_travel_time`,
                cqad.`rsa_ic_id`,
                cqad.`question_id`,
                vcr.`task_id`,
                vcr.`vendor_contact_no`,
                vcr.`task_start_time`,
                vcr.`reach_status`,
                vcr.`reach_time`,
                vcr.`task_completed_status`,
                vd.`member_Name`,
                vd.`email_id`,
                vcr.`reach_date` 
                FROM
                customer_ques_ans_details cqad 
                LEFT OUTER JOIN vendor_current_record vcr 
                ON cqad.id = vcr.task_id 
                LEFT OUTER JOIN vendor_details vd 
                ON vd.id = vcr.vendor_id 
                LEFT OUTER JOIN tvs_status_details tsd 
                ON cqad.rsa_status_id = tsd.id
                $where
                 ORDER BY cqad.id DESC
        ";
        // die($sql);
        $result =  $this->db->query($sql)->result_array();
        return $result ;
     ...
     function checkPolicyKey($key)
    {
         $sql = "SELECT cqad.`policy_no`,tsd.`status_code`,tsd.`id`
                    FROM customer_ques_ans_details cqad
                    LEFT OUTER JOIN tvs_status_details tsd 
                    ON cqad.`rsa_status_id` = tsd.id WHERE json_extract(`policy_details`, '$[0].sold_policy_no')  like '%$key%' 
                    OR json_extract(`policy_details`, '$[0].engine_no')  = '$key' 
                    OR  json_extract(`policy_details`, '$[0].chassis_no') = '$key'
                    ORDER BY cqad.id DESC LIMIT 1 
                     ";
