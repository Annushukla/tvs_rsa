            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="example-col">
                        <table class="table table-bordered table-striped w-full" id="pending_dealer_payment_datatable">
                            <thead>
                            <th>Sr.no</th>
                            <th>Dealer Name</th> 
                            <th>SAP Ad Code</th>
...
<div id="dealerPaymentPopUp" class="modal fade" role="dialog">
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            ...
            <button type="button" class="btn btn-default" id="paycommission">Submit</button>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        table = $('#pending_dealer_payment_datatable').DataTable({
            ...
            "ajax": {
                'url': base_url + 'admin/pendingDealerPayment_ajax',
                "type": "POST",
                "dataType": "json",
    ...
    function dealerPayment(dealer_id,total_commission){
        $('#commission').text(total_commission);
        $('#amount').val(total_commission);
        $('#dealer_id').val(dealer_id);

        $.ajax({
            url:base_url +'admin/get_dealer_bank_tran_id',
            data:{'dealer_id':dealer_id},
            dataType:'JSON',
            type:'POST',
            success:function(response){
                //console.log('Id '+response.id);
                $('#dealer_bank_tran_id').val(response.id);
            }    
        });    

        $("#dealerPaymentPopUp").modal('show');
    }
