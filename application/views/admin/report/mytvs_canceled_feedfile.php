<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>MY TVS Canceled Feedfile Report Download</h1>
...
function download_csv()
  ...
  if(status==true){
        if(from_date <= to_date){
              $.ajax({
              url : base_url+"Report/download_mytvs_canceled_feedfile/"+datas,
              type: "POST",
              success: function(data){
                if(data!=""){
                  alert('Feed-File Downloaded');
                }
             window.location=base_url+"Report/download_mytvs_canceled_feedfile/"+datas;
              }
            });
        }else{
