...
function download_csv()
  ...
  if(status==true){
        if(from_date <= to_date){
              $.ajax({
              url : base_url+"Report/download_pa_canceled_feedfile/"+datas,
              type: "POST",
              success: function(data){
                if(data!=""){
                  alert('Feed-File Downloaded');
                }
             window.location=base_url+"Report/download_pa_canceled_feedfile/"+datas;
              }
            });
        }else{
