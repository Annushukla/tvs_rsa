<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
        <?php if(isset($endorse)){
         echo "Reliance General Insurance ".ucfirst($endorse)." Feed-File";
        }else{
          echo "Reliance General Insurance Feed-File";
        }?>
      </h1>
...
function download_csv()
 ...
 var status = true;
  ...
  if(status==true){
        if(from_date <= to_date){
              $.ajax({
              url : base_url+"Report/DownloadRelianceFeedfile/"+datas,
              type: "POST",
              success: function(data){
                if(data!=""){
                  alert('Feed-File Downloaded');
                }
             window.location=base_url+"Report/DownloadRelianceFeedfile/"+datas;
              }
            });
        }else{
