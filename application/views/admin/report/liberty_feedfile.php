    ...
    <section class="content-header">
      <h1>
        <?php if(isset($endorse)){
         echo "Liberty General Insurance ".ucfirst($endorse)." Feed-File";
        }else{
          echo "Liberty General Insurance Feed-File";
        }?>
...
function download_csv()
 ...
 var status = true;
  ...
  if(status==true){
        if(from_date <= to_date){
              $.ajax({
              url : base_url+"Report/DownloadLibertyFeedfile/"+datas,
              type: "POST",
              success: function(data){
                if(data!=""){
                  alert('Feed-File Downloaded');
                }
             window.location=base_url+"Report/DownloadLibertyFeedfile/"+datas;
              }
            });
        }else{
