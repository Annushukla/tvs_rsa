...
$(document).ready(function(){
	jQuery.validator.addMethod("uniqueName", function(value, element) {
		...
		$.ajax({
	        type: "POST",
	        async:false,
	        url: base_url+'admin/AdminController/checkDuplicateState',
	        dataType : "json",
    ...
    table = $('.data_tables').DataTable({ 
        ...
        "ajax": {
        	'url' : base_url+'admin/AdminController/ajaxStates',
            "type": "POST",
            "dataType": "json",
    ...
    $('.open_model').click(function(){
 	...
 	if(!$( "#form_update" ).valid()){ return false; }else{
 		...
 		$.ajax({
			url: base_url+'admin-states-update',
...
function editModel(id){
	...
	$.ajax({
		url: base_url+'admin-states-edit',
		data : { 'id' : id},
		dataType: 'Json',
		type: 'POST',
		success: function(response){
...
function deleteRecord(id){
	...
	if(confirm('Are you sure delete this data?')){
		...
		$.ajax({
			url: base_url+'admin-states-delete',
			data : { 'id' : id},
			dataType: 'Json',
			type: 'POST',
			success: function(response){
