...
$(document).ready(function(){
	jQuery.validator.addMethod("uniqueName", function(value, element) {
	    ...
	    $.ajax({
	        type: "POST",
	        async:false,
	        url: base_url+'admin/AdminController/checkDuplicateCity',
	        dataType : "json",
	        data : { 'name' : name,'state' : state,'id' : id},
	...
	$("#form_update").validate({
    	...
    	errorClass: 'err',
	  	...
	  	messages: {
			state : {
				required: 'Select State required.'
			},
			name : {
    ...
    table = $('.data_tables').DataTable({ 
        ...
        "ajax": {
        	'url' : base_url+'admin/AdminController/ajaxCities',
            "type": "POST",
            "dataType": "json",
    ...
    $('.open_model').click(function(){
 	...
 	if(!$( "#form_update" ).valid()){ return false; }else{
 		...
 		$.ajax({
			url: base_url+'admin-cities-update',
...
function editModel(id){
	...
	$.ajax({
		url: base_url+'admin-cities-edit',
		data : { 'id' : id},
		dataType: 'Json',
		type: 'POST',
		success: function(response){
			resetForm();
			$('#pop_heading').html('Update City');
			//alert(response.result[0].state);
			$('#name').val(response.result[0].name);
			$('#state').val(response.result[0].state_id);
