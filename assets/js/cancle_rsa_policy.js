...
reject_cancel_policy_list = $('#reject_cancel_policy_list').DataTable({ 
           "scrollX": true,
          "processing": true,
          'paging'      : true,
          'lengthChange': true,
        ...
        // Load data for the table's content from an Ajax source
        "ajax": {
            'url' : base_url+'RejectCancelPolicyAjax',
            "type": "POST",
            "dataType": "json",
            "dataSrc": function (jsonData) {
                    return jsonData.data;
  ...
  var to_date = $('#to_date').val();
    ...
    if(status_er == false){
      ...
      date2 = new Date(to_date);
        ...
        if(date1 <= date2){
              cancle_rsa_policy_table = $('#cancle_rsa_policy_table').DataTable({
                'paging': true,
                'destroy': true,
                // Load data for the table's content from an Ajax source
                "ajax": {
