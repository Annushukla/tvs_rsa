...
$("#reject_invoice").on('click',function(){
       ...
       $("#approveDealer").on('click',function(){
        ...
        var transaction_type =   $("#transaction_type").val();
            if(transaction_type =='withdrawal'){
                 ...
                 admin_trans_no = admin_trans_no.trim();

                 if(admin_trans_no == ''){
                    $('#error-admin_trans_no').text('Please Enter transaction no.');
                    return;
                 }
            }else{
                    admin_trans_no='';   
 ...
 $("#paycommission").on('click',function(){
    var dealer_bank_tran_id =   $("#dealer_bank_tran_id").val();
        var dealer_id =   $("#dealer_id").val();
        var amount =   $("#amount").val();
        var transaction_type =   $("#transaction_type").val();
            if(transaction_type =='withdrawal'){
                ...
                $.ajax({
                url:base_url +'admin/paycommission',
                data:{'dealer_bank_tran_id':dealer_bank_tran_id, 'dealer_id': dealer_id,'amount':amount,
                        'transaction_type':transaction_type,admin_trans_no:admin_trans_no},
                dataType:'JSON',
                type:'POST',
        ...
        $.ajax({
            ...
            success : function(response){
                if(response!=""){
                    ...
                    $('.cancel_policy_count_body').html(response.html2);
